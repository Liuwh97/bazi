...
def run_bazi(argv=None, *, capture_output=False):
    """运行八字排盘；capture_output=True 时返回 stdout 文本。"""
    if capture_output:
        buffer = io.StringIO()
...
def compute_bazi(argv=None):
    """兼容别名：返回 run_bazi(argv, capture_output=True) 的结果。"""
    return run_bazi(argv=argv, capture_output=True)
